...
const (
	...
	DefaultPaymentServiceURL  = "http://192.168.33.9:5555"
	DefaultShipmentServiceURL = "http://192.168.33.9:7000"
...
func getUserSimpleByIDs(q sqlx.Queryer, userIDs []int64) (userSimpleMap map[int64]UserSimple, err error) {
	qs, params, err := sqlx.In("SELECT id, account_name, num_sell_items FROM `users` WHERE `id` IN (?)", userIDs)
	if err != nil {
		return userSimpleMap, err
	}

	userSimples := []UserSimple{}
	if err := sqlx.Select(q, &userSimples, qs, params...); err != nil {
		return userSimpleMap, err
	}

	userSimpleMap = make(map[int64]UserSimple)
	for _, user := range userSimples {
		userSimpleMap[user.ID] = user
	}
...
func getTransactions(w http.ResponseWriter, r *http.Request) {
	...
	if itemID > 0 && createdAt > 0 {
		// paging
		err := tx.Select(&items,
			"SELECT * FROM (SELECT * FROM `items` WHERE `seller_id` = ? UNION ALL SELECT * FROM `items` WHERE `buyer_id` = ?) TMP WHERE `status` IN (?,?,?,?,?) AND (`created_at` < ?  OR (`created_at` <= ? AND `id` < ?)) ORDER BY `created_at` DESC, `id` DESC LIMIT ?",
			user.ID,
			user.ID,
			ItemStatusOnSale,
			ItemStatusTrading,
		...
		// 1st page
		err := tx.Select(&items,
			"SELECT * FROM (SELECT * FROM `items` WHERE `seller_id` = ? UNION ALL SELECT * FROM `items` WHERE `buyer_id` = ?) TMP WHERE `status` IN (?,?,?,?,?) ORDER BY `created_at` DESC, `id` DESC LIMIT ?",
			user.ID,
			user.ID,
			ItemStatusOnSale,
			ItemStatusTrading,
